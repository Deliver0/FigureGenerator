...
const App = () => {
  ...
  const [roi, setRoi] = useState({ x: 100, y: 100, width: 100, height: 100 });
  ...
  const sketch = (p) => {
    ...
    p.mousePressed = () => {
      if (image && p.mouseX > 0 && p.mouseX < 400 && p.mouseY > 0 && p.mouseY < 400) {
        ...
        if (
          p.mouseX > roi.x + roi.width - 10 &&
          p.mouseX < roi.x + roi.width + 10 &&
          p.mouseY > roi.y + roi.height - 10 &&
          p.mouseY < roi.y + roi.height + 10
        ) {
          resizing = true;
        }
        // Check if clicking inside ROI for dragging
          ...
          p.mouseX > roi.x &&
          p.mouseX < roi.x + roi.width &&
          p.mouseY > roi.y &&
          p.mouseY < roi.y + roi.height
        ) {
          dragging = true;
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">Interactive Scientific Figure Generator</h1>
      <div className="flex flex-col md:flex-row gap-4">
        <div>
          <div id="canvas-container" className="mb-4"></div>
          <input
            type="file"
            accept="image/*"
        ...
        <div className="flex flex-col gap-4">
          <div>
            <label className="block">Zoom Level: {zoom.toFixed(1)}x</label>
            <input
              type="range"
              min="1"
              max="5"
          ...
          <div>
            <label className="block">ROI X: {roi.x.toFixed(0)}</label>
            <input
              type="range"
              min="0"
              max="350"
              value={roi.x}
              onChange={(e) => setRoi({ ...roi, x: parseFloat(e.target.value) })}
              className="w-full"
            />
          </div>
          <div>
            <label className="block">ROI Y: {roi.y.toFixed(0)}</label>
            <input
              type="range"
              min="0"
              max="350"
              value={roi.y}
              onChange={(e) => setRoi({ ...roi, y: parseFloat(e.target.value) })}
              className="w-full"
            />
          </div>
          <div>
            <label className="block">ROI Width: {roi.width.toFixed(0)}</label>
            <input
              type="range"
              min="50"
              max="200"
              value={roi.width}
              onChange={(e) => setRoi({ ...roi, width: parseFloat(e.target.value) })}
              className="w-full"
            />
          </div>
          <div>
            <label className="block">ROI Height: {roi.height.toFixed(0)}</label>
            <input
              type="range"
              min="50"
              max="200"
              value={roi.height}
              onChange={(e) => setRoi({ ...roi, height: parseFloat(e.target.value) })}
              className="w-full"
            />
          </div>
        </div>
